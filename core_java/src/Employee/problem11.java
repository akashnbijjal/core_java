...
public class problem11 {
	...
	public static void main(String[] args) {
		// Calculate the average salary of employees in each department and find the
		...
		Map<Integer, Double> averagesalary = emp.stream()
				.collect(Collectors.groupingBy(Employee::getDeptid, Collectors.averagingDouble(Employee::getSalary)));

		double highestaveragesalary = averagesalary.values().stream().mapToDouble(Double::doubleValue).max()
				.orElse(0.0);
		

		averagesalary.entrySet().forEach(entry -> System.out.println(entry.getKey() + "--->" + entry.getValue())

		);
		System.out.println(highestaveragesalary);
